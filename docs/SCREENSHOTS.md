...
| `scripts/strip_alpha.py` | PNG 알파 채널 제거(App Store 업로드 필수, 스크립트에서 자동 호출) |
...
### 알파 채널 주의 (App Store Connect)

App Store Connect는 **알파 채널(투명도)이 포함된 스크린샷을 거부**합니다
(`이미지는 알파 채널 또는 투명도를 포함할 수 없습니다`). `takeScreenshot`이 만든
PNG는 RGBA(알파 포함)이므로, 촬영 스크립트가 끝에서 `strip_alpha.py`로 자동
제거합니다. 다른 경로로 만든 이미지는 다음으로 직접 제거할 수 있습니다.

```bash
python3 scripts/strip_alpha.py screenshots   # 디렉터리(재귀) 또는 개별 파일
```
